...
func (s *KeeperTestSuite) TestGetProportions() {
	...
	tests := []struct {
		...
		},
		{
			...
			// Ref: https://github.com/arnac-io/osmosis/issues/1917
	...
	for name, tc := range tests {
		s.Run(name, func() {
			...
			osmoassert.ConditionalPanic(s.T(), tc.expectPanic, func() {
				...
				// To be fixed: https://github.com/arnac-io/osmosis/issues/2025
				...
				// TODO: these should be equal, slightly off due to known rounding issues: https://github.com/arnac-io/osmosis/issues/1917
				// s.Require().Equal(oldDeveloperVestingModuleBalanceAmount.Sub(expectedDistributed).Int64(), actualDeveloperVestingModuleBalanceAmount.Int64())

				expectedDistributedCommunityPool := sdk.NewInt(0)

				for i, weightedAddress := range tc.recepientAddresses {
					// TODO: truncation should not occur: https://github.com/arnac-io/osmosis/issues/1917
